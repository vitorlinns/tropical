    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 relative">
        ...
        <div className="max-w-xl mb-16">
          <h2 id="how-title" className="font-display font-bold text-4xl md:text-5xl text-ink leading-tight">
            <AnimatedText text="Três passos para" className="block" />
            <AnimatedText text="usar suas milhas" className="text-brand block" delay={0.1} />
          </h2>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-6">
                  ...
                  <span className="font-display font-bold text-4xl text-surface-2" aria-hidden="true">
                    {step.number}
                  </span>
                </div>
                <h3 className="font-display font-bold text-xl text-ink mb-3">{step.title}</h3>
                <p className="text-ink-4 text-sm leading-relaxed">{step.description}</p>
                {i < steps.length - 1 && (
        ...
        >
          ...
          <p className="text-sm" style={{ color: "var(--color-muted)" }}>Passagem emitida em até 24h úteis</p>
        </motion.div>
      </div>
    </section>
  );
