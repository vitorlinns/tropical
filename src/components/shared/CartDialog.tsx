  ...
  return (
    <>
      ...
      <motion.div
        ...
        style={{ backgroundColor: colors.white, overscrollBehavior: "contain", boxShadow: "-8px 0 32px rgba(0,0,0,0.08)" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: `1px solid ${colors.border}` }}>
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: colors.brandLight }}>
              <RiShoppingBag3Line size={17} style={{ color: colors.brand }} aria-hidden="true" />
            </div>
            <div>
              <h2 className="font-display font-bold text-base leading-tight" style={{ color: colors.ink }}>
                Minhas compras
              </h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 gap-3 text-center">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: colors.surface }}>
                <RiFlightTakeoffLine size={26} style={{ color: colors.mutedLighter }} aria-hidden="true" />
              </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="font-display font-bold text-base" style={{ color: colors.ink }}>
                        {item.flight.originCode}
                      </span>
                      <div className="flex items-center gap-1" style={{ color: colors.mutedLighter }}>
                        <div className="h-px w-5" style={{ backgroundColor: colors.border }} />
                    ...
                    <button
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = colors.dangerLight; e.currentTarget.style.color = colors.danger; }}
                      onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = colors.mutedLighter; }}
                  ...
                  <p className="font-semibold text-sm mb-1" style={{ color: colors.ink }}>
                  ...
                  <div className="grid grid-cols-3 gap-2 rounded-xl p-3" style={{ backgroundColor: colors.surface }}>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider mb-0.5" style={{ color: colors.mutedLight }}>Companhia</p>
                      <p className="text-xs font-semibold truncate" style={{ color: colors.ink }}>{item.flight.airline}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider mb-0.5" style={{ color: colors.mutedLight }}>Classe</p>
                      <p className="text-xs font-semibold" style={{ color: colors.ink }}>{item.flight.flightClass}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider mb-0.5" style={{ color: colors.mutedLight }}>Duração</p>
                      <p className="text-xs font-semibold flex items-center gap-0.5" style={{ color: colors.ink }}>
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: `1px solid ${colors.surface2}` }}>
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="px-6 py-5 space-y-3" style={{ borderTop: `1px solid ${colors.border}` }}>
            ...
            <button
              ...
              onMouseLeave={(e) => { e.currentTarget.style.color = colors.mutedLight; }}
            >
              Limpar seleção
            </button>
          </div>
